...
export async function GET(req: NextRequest) {
  try {
    ...
    const qualifiedLeads = allLeads.filter(
      (lead) => lead.tag === "QUALIFIED" || lead.tag === "HOT"
    ).length;
    ...
    const convertedLastMonth = leadsLastMonth.filter(
      (lead) => lead.tag === "QUALIFIED" || lead.tag === "HOT"
    ).length;

    const lastMonthConversionRate =
      leadsLastMonth.length > 0
        ? (convertedLastMonth / leadsLastMonth.length) * 100
        : 0;
    ...
    const leadsByTag = {
      ...
      DISQUALIFIED: allLeads.filter((lead) => lead.tag === "DISQUALIFIED")
        .length,
    };
      ...
      data: {
        metrics: {
          totalRevenue: {
            ...
            subtitle:
              revenueChange >= 0 ? "Trending up this month" : "Down this month",
          },
          newLeads: {
            value: leadsThisWeek,
            formatted: leadsThisWeek.toString(),
            change: `${newLeadsChange >= 0 ? "+" : ""}${newLeadsChange.toFixed(
              1
            )}%`,
          ...
          activeEmployees: {
            ...
            change: `${employeeChange >= 0 ? "+" : ""}${employeeChange.toFixed(
              1
            )}%`,
          ...
          conversionRate: {
            ...
            }${conversionRateChange.toFixed(1)}%`,
